        ...
        for j in range(len_A_figure):  # цикл по всем границам отсекаемого многоугольника
            if j == 0:
                ...
                if is_crossing == True:
                    ...
                    # если пересечене в точке s или p (нужно, чтобы учесть погрешность)
                    if visibility(s, C_clipper[i], C_clipper[i + 1]) == 0:
                        res_B_figure.append(s)
                        len_res_B_figure += 1
                    elif visibility(A_figure[j], C_clipper[i], C_clipper[i + 1]) == 0:
                        res_B_figure.append(s)
                        len_res_B_figure += 1
