...
from Point import *
import math as m
from canonic import canonic_eq_circle,canonic_eq_ellipse
from param import param_eq_circle, param_eq_ellipse
from brezenhem import brezenhem_ellipse, brezenhem_circle
from midpoint import alg_midpoint_circle, alg_midpoint_ellipse

def mathround(n):
    return int(n + 0.5)


def canonic_eq(ra, rb, center: Point, canvas, color, draw, count_loop=False):
    if ra == rb:
        return canonic_eq_circle(ra, center, canvas, color, draw)
    else:
        return canonic_eq_ellipse(ra, rb, center, canvas, color, draw, count_loop)


def param_eq(ra, rb, center: Point, canvas, color, draw, count_loop=False):
    if ra == rb:
        return param_eq_circle(ra, center, canvas, color, draw)
    else:
        return param_eq_ellipse(ra, rb, center, canvas, color, draw, count_loop)


def brezenhem(ra, rb, center: Point, canvas, color, draw, count_loop=False):
    if ra == rb:
        return brezenhem_circle(ra, center, canvas, color, draw)
    else:
        return brezenhem_ellipse(ra, rb, center, canvas, color, draw, count_loop)


def alg_midpoint(ra, rb, center: Point, canvas, color, draw, count_loop=False):
    if ra == rb:
        return alg_midpoint_circle(ra, center, canvas, color, draw)
    else:
        return alg_midpoint_ellipse(ra, rb, center, canvas, color, draw, count_loop)
